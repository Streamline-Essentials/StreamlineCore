...
//                tc.addExtra(new LiteralText(after).styled(style -> style.withColor(Integer.decode(colorHex))));
    ...
    public TreeMap<Integer, Player> getTaggedPlayersIndexed(String[] args, String serverName) {
        ...
        List<Player> players = Streamline.getInstance().playersOnServer(serverName);
    ...
    public String replaceAllPlayerBungee(CommandSender sender, String of) {
        return replaceAllPlayerBungee(SLAPI.getInstance().getUserManager().getOrGetUserByName(sender.getName()), of);
    }
