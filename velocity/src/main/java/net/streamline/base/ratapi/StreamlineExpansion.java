...
public class StreamlineExpansion extends RATExpansion {
    ...
    public String onLogic(String params) {
        ...
        if (params.matches("([a][u][t][h][o][r][\\[]([0-2])[\\]])")) {
            ...
            while (matcher.find()) {
                Optional<PluginContainer> container = Streamline.getInstance().getProxy().getPluginManager().getPlugin("streamlineapi");
                if (container.isEmpty()) return null;
                return container.get().getDescription().getAuthors().get(0);
            }
        }
