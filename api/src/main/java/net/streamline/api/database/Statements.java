...
public class Statements {
    ...
    public enum MySQL {
        ...
        PUSH_PLAYER_MAIN("INSERT INTO `%table_prefix%players` (" +
                ...
                ") VALUES (" +
                "'%uuid%', '%first_join%', '%last_join%', '%current_name%', '%current_ip%', %play_seconds%, %points%, %proxy_touched%" +
                ") ON DUPLICATE KEY UPDATE  " +
                "FirstJoin = '%first_join%', LastJoin = '%last_join%', CurrentName = '%current_name%', CurrentIP = '%current_ip%', " +
                "PlaySeconds = %play_seconds%, Points = %points%, ProxyTouched = %proxy_touched%;"),
        ...
        PLAYER_IS_TOUCHED("SELECT ProxyTouched FROM `%table_prefix%players` WHERE Uuid = '%uuid%';"),
        ;
