 ...
 */
public abstract class StreamlineListener implements ExtensionPoint {
    @Getter @Setter
    ...
    public StreamlineListener() {
        setIndex(getListeners().size());
        getListeners().put(getIndex(), this);
        setMethods(new ArrayList<>(getConcurrentMethods().values()));
    }

    public StreamlineListener(List<Method> methods) {
        setIndex(getListeners().size());
        getListeners().put(getIndex(), this);
        setMethods(methods);
    }
    ...
    public ConcurrentHashMap<Class<? extends StreamlineEvent>, ConcurrentSkipListSet<RegisteredListener<?>>> setUp(ModuleLike module) {
        ...
        for (final Method method : getMethods()) {
            ...
            if (module instanceof StreamlineModule sm) eventSet.add(new RegisteredListener<>(this, executor, eh.priority(), sm, eh.ignoreCancelled()));
            if (module instanceof StreamlineSpringModule ssm) eventSet.add(new RegisteredListener<>(this, executor, eh.priority(), ssm, eh.ignoreCancelled()));
        }
        return ret;
    }
    ...
    public ConcurrentHashMap<Class<? extends StreamlineEvent>, ConcurrentSkipListSet<SpringRegisteredListener>> setUp(StreamlineSpringModule module) {
        ...
        for (final Method method : getMethods()) {
            ...
            eventSet.add(new SpringRegisteredListener(this, executor, eh.priority(), module, eh.ignoreCancelled()));
        }
        return ret;
    }
}
