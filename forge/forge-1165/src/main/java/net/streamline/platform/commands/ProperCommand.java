...
public class ProperCommand implements IProperCommand {
    ...
    public LiteralArgumentBuilder<CommandSource> buildBrigadier() {
        LiteralArgumentBuilder<CommandSource> base = Commands.literal(parent.getBase())
                .executes(ctx -> execute(ctx, new String[0]));

        RequiredArgumentBuilder<CommandSource, String> argsNode =
    ...
    private int execute(CommandContext<CommandSource> ctx, String[] args) {
    ...
    private static CosmicSender resolveCosmicSender(CommandSource source) {
